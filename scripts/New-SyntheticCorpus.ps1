...
foreach ($caseNumber in 1..5) {
    $sessionId = ('11111111-1111-4111-8111-{0:D12}' -f $caseNumber)
    $project = Join-Path $corpusRoot ('project-' + $(if ($caseNumber -eq 4) {'beta'} elseif ($caseNumber -eq 5) {'subagent'} else {'alpha'}))
    New-Item -ItemType Directory -Force -Path $project | Out-Null
    $sessionMeta = @{id=$sessionId; timestamp='2026-01-01T00:00:00.000Z'; cwd=$project; originator='codex_cli_rs'; cli_version='0.152.1'; source='cli'; model_provider='mock'; base_instructions=@{text='Synthetic compatibility test. Answer briefly.'}}
    if ($caseNumber -eq 5) {
        $sessionMeta.thread_source = 'subagent'
        $sessionMeta.parent_thread_id = '11111111-1111-4111-8111-999999999999'
    }
    $records = @(New-Record 'session_meta' $sessionMeta)
    ...
    $expected = if ($caseNumber -eq 4) {'ARCHIVE_ONLY'} elseif ($caseNumber -eq 5) {'COMPACT_REFUSED'} else {'COMPACT_ALLOWED'}
    $fixtureCases += @{name=('synthetic-case-' + $caseNumber); session_id=$sessionId; path=$path; expected=$expected}
}
